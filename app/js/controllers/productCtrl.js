	...
	function ($scope, $routeParams, $route, $location, $451, Product, ProductDisplayService, Order, Variant, User) {
		...
		$scope.addToOrder = function(){
			...
			if ($scope.currentOrder && $scope.currentOrder != null && $scope.currentOrder.LineItems.length) {
				...
				if (itemFound === true) {
					if ($scope.LineItem.Product.StaticSpecGroups && $scope.LineItem.Product.StaticSpecGroups.isapparel) {
						//if this is apparel
						//alert("Yes, add this apparel item to my apparel cart.");
						addItemsToOrder();
					}

					//if this is not apparel
					else {
						alert("Your cart contains apparel items which cannot be combined with print or promo items.");
